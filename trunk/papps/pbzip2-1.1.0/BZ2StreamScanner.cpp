 ...
 * On append [current, searchPos) is appended to output buffer. Output buffer is
 ...
 */
int BZ2StreamScanner::appendOutBuffData()
{
	int additionSize = getInBuffSearchPtr() - getInBuffCurrent();
	...
	_inBuffCurrent = getInBuffSearchPtr();
...
BZ2StreamScanner::CharType * BZ2StreamScanner::searchNextHeader()
{
	...
	while ( !getSearchStatus() && !eof() && !failed() )
	{
		readData();
...
outBuff BZ2StreamScanner::getNextStream()
{
	...
	if ( !failed() && ( getInBuffSearchPtr() == getInBuffEnd() ) )
	{
		if ( !eof() )
		{
			// impossible situation - if we're unable to find next header
			// we should be at end of file
			_errState |= ERR_INVALID_STATE;
			return res;
		}
	...
	if ( !failed() && isBz2HeaderFound() )
	{
		// It's ensured search pos is positioned on header beginning.
		// Slide a bit to skip it in order to search for next one.
		// (Here we rely on fact first header letter is unique)
	...
	fprintf( stderr, " end getNextStream/blockRange=[%"PRIu64", %"PRIu64"): [", startBlock, blockNum );
	printCurrentState();
	fprintf( stderr, "\n" );
	#endif
